...
static void DefineDht11Object(UA_Server *server){
    UA_NodeId dht11Id;
...
static void createDht11ObjectInstance(UA_Server *server, struct dht11InitInfo *info){
    ...
    targetNameArr[0] = UA_QUALIFIEDNAME(1, "Tmp");   
    ...
    targetNameArr[0] = UA_QUALIFIEDNAME(1, "Rh");   
    if (getChildIdSimplified(server, retId, 1, targetNameArr, &itemId) == 0)
    {
        UA_Variant_init(&value);
